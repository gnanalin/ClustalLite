...
def calculate_score_multiple_alignment(matrix, current_line, current_col, cluster1_seq_list, cluster2_seq_list):
    ...
    cluster1_seq_list : list of sequences from cluster 1
    cluster2_seq_list : list of sequences from cluster 2
    ...
    the diagonal, the diagonal box will be selected and otherwise,
    it left box is selected
    """
    mean_diag_score = 0
    counter = 0
...
def compute_multiple_alignment(matrix_align, cluster1_list, cluster2_list):
    ...
    cluster1_alignments, cluster2_alignments = [
        ...
        match matrix_align[current_box_i, current_box_j]:
            ...
            case 0:
                for i in range(len(cluster1_alignments)):
                    cluster1_alignments[i] = (
                        cluster1_list[i][current_box_i - 1] + cluster1_alignments[i]
                    )
                for i in range(len(cluster2_alignments)):
                    cluster2_alignments[i] = (
                        cluster2_list[i][current_box_j - 1] + cluster2_alignments[i]
                    )
                current_box_i -= 1
                current_box_j -= 1
            # left
...
def compute_cluster_alignment(group, dict_multiple_alignment):
    ...
    cluster1_seq_list, cluster2_seq_list = (
        dict_multiple_alignment[group_0],
        dict_multiple_alignment[group_1],
    )
    ...
    for i in range(1, nlin):
        for j in range(1, ncol):
            score_align, index_max = calculate_score_multiple_alignment(
                matrix, i, j, cluster1_seq_list, cluster2_seq_list
            )
            matrix[i, j] = score_align
            matrix_align[i, j] = index_max
    return compute_multiple_alignment(
        matrix_align, cluster1_seq_list, cluster2_seq_list
    )
